...
const DashboardOverClientView = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <Card className="overflow-hidden rounded-[30px] border-0 bg-[#182235] text-white shadow-[0_25px_60px_-35px_rgba(15,23,42,0.85)]">
          ...
          <div className="relative flex flex-col gap-6 p-6 sm:p-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="max-w-3xl space-y-4">
              <div className="flex flex-wrap items-center gap-3">
                <h2 className="text-3xl font-bold font-poppins leading-tight text-white">
            ...
            <div className="flex w-full flex-col gap-4 sm:w-auto sm:flex-row lg:flex-col">
              <Button
                className="h-20 min-w-[132px] rounded-2xl bg-white px-8 text-lg font-bold text-slate-950 hover:bg-white/95"
                onClick={() => navigate("/client/loyalty")}
              >
                Mes Points
              </Button>
              <Button
                className="h-20 min-w-[172px] rounded-2xl bg-[#316BFF] px-8 text-lg font-bold text-white hover:bg-[#2558db]"
                onClick={() => navigate("/client/loyalty")}
              >
                Parrainer un ami
              </Button>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-[30px] font-bold tracking-tight text-slate-900 font-poppins">
            Locations Récentes
          </h3>
        </div>
          ...
          {recentReservations.length > 0 ? (
            recentReservations.map((reservation) => {
              ...
              return (
                <Card
                  key={reservation.id}
                  className="rounded-[28px] border border-slate-200/80 bg-white shadow-[0_16px_45px_-35px_rgba(15,23,42,0.45)]"
                >
                  <CardContent className="flex flex-col gap-5 p-5 sm:p-6 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex min-w-0 items-center gap-4 sm:gap-5">
                      ...
                      <div className="min-w-0 space-y-1.5">
                        <h4 className="truncate text-2xl font-bold text-slate-900 font-poppins">
                          {vehicle?.titre || `${vehicle?.marque_data?.nom || "Véhicule"} ${vehicle?.modele_data?.label || ""}`.trim()}
                        </h4>
                        <p className="text-xl text-slate-500">{locationLabel}</p>
                        <div className="flex flex-wrap items-center gap-3 text-sm text-slate-400 sm:text-base">
                    ...
                    <div className="flex flex-col items-start gap-3 lg:items-end">
                      <Button
                        className="rounded-2xl bg-[#EEF4FF] px-6 text-base font-semibold text-[#316BFF] hover:bg-[#E2ECFF]"
                      ...
                      <button
                        type="button"
                        className="text-base font-medium text-slate-400 transition-colors hover:text-slate-600"
            ...
            <Card className="rounded-[28px] border border-dashed border-slate-300 bg-white shadow-sm">
      ...
      <section>
        <Card className="rounded-[30px] border border-slate-200/80 bg-white shadow-[0_16px_45px_-35px_rgba(15,23,42,0.45)]">
          <CardContent className="p-6 sm:p-8">
            ...
            <div className="space-y-4">
              {documentItems.map((document) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xl font-semibold text-slate-900">{document.label}</p>
                      <p className="text-base text-emerald-600">{document.status}</p>
                    </div>
                  </div>
              ...
              <button
                type="button"
                className="flex w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 px-4 py-5 text-lg font-medium text-slate-500 transition-colors hover:border-primary/30 hover:text-primary"
